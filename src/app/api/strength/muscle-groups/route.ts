...
export async function GET() {
  try {
    ...
    const { data, error } = await db.from("strength_muscle_groups")
      .select("id, tenant_id, name, body_region, created_at")
      .order("body_region").order("name");
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.insert("strength_muscle_groups", { name: name.trim(), body_region: body_region ?? null }).select().single();
    if (error) throw error;
