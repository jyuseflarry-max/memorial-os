    ...
    <div className="border-b border-gray-700/50 last:border-0">
      ...
      >
        ...
        <div className="flex-1 bg-gray-700 rounded-full h-3 overflow-hidden">
          <div className={`h-full rounded-full transition-all duration-500 ${barColor}`} style={{ width: `${row.percentage}%` }} />
        </div>
        ...
        <div className="bg-gray-800/30 border-t border-gray-700/30">
          {row.sub_categories.map((sub) => (
            <div key={sub.sub_category} className="flex items-center gap-4 px-4 py-2.5 border-b border-gray-700/20 last:border-0">
              ...
              <span className="text-gray-400 text-xs w-36 shrink-0 font-mono truncate">{sub.sub_category}</span>
              ...
              <div className="flex items-center gap-6 shrink-0 text-right">
                <span className="text-gray-300 font-mono text-xs w-16 text-right">
                  {sub.percentage}%<span className="text-gray-600 text-[10px]"> total</span>
                </span>
...
export default function PracticeTimeReportPage() {
  ...
  return (
    <DashboardLayout>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-white text-2xl font-bold tracking-tight">Practice Time</h1>
      ...
      {report && report.categories.length > 0 && (
        <p className="text-[10px] font-mono text-gray-600 mt-3 text-right">
          Expand a category to see individual drill breakdowns
        </p>
      )}
    </DashboardLayout>
  );
